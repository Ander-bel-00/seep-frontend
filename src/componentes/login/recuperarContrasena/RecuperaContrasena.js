...
function RecuperaContrasena() {
  ...
  const handleCambiarContrasena = async (e) => {
    ...
    try {
      ...
      setError("Error al cambiar contraseña: " + JSON.stringify(error.response.data));
    }
  };
    ...
    <div className="login-body">
          ...
          {!codigoEnviado && showSolicitarCodigo ? (
            <div className="recupera-formulario">
              <form onSubmit={handleSolicitarCodigo}>
                <label htmlFor="numero_documento"><strong>Ingresar Número de Documento</strong></label>
                <input
                ...
                />
                <label htmlFor="correo_electronico1"><strong>Ingresar Correo Electrónico</strong></label>
                <input
                  className="recupera-input"
                  type="email"
      ...
      <div className="inicio-box-main">
        <Link className="link-home" to="/login">
          <IoHomeSharp className="inline-block" /> <p className="inline-block relative top-1">Ir al Inicio</p>
        </Link>
      </div>
    </div>
  );
